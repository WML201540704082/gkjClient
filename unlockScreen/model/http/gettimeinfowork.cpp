...
#include <QThread>
#include <QDebug>
extern std::atomic<bool> getTimeInfoFinished;

getTimeInfoWork::getTimeInfoWork(QObject *parent) : QObject(parent)
{
    
}

getTimeInfoWork::~getTimeInfoWork()
{
    if(!client)
    {
        delete client;
...
void getTimeInfoWork::recheckTimestamps()
{
    ...
    connect(this, &getTimeInfoWork::startGetTimestamps, tmpTimestampsWork, &getTimestampsWork::startSending);
    connect(tmpTimestampsWork, &getTimestampsWork::calculateSuccess, this, &getTimeInfoWork::onCalculateSuccess);
...
void getTimeInfoWork::processData(QJsonValue value)
{
    QList<usageTime> serverTimeList;
    if(value.toString().isEmpty())
    {
        emit getUsageTimeInfoSuccessfully(serverTimeList);
        return;
    }
...
void getTimeInfoWork::startSending()
{
    if(!client)
    {
        delete client;
    ...
    //拼接json
    QJsonObject root;
    root["method"] = "downTime";
    root["data"] = "";
    
    //初始化client并发送
    client = new httpClient();
    connect(client, &httpClient::requestSuccess, this, &getTimeInfoWork::handleSuccess);
    connect(client, &httpClient::requestError, this, &getTimeInfoWork::handleError);
    connect(client, &httpClient::sendFail, this, &getTimeInfoWork::handleFail);
    
    QUrl url = QUrl(urlCommon + "/downTime");
    client->post(url, QJsonDocument(root).toJson(QJsonDocument::Compact));
}

void getTimeInfoWork::handleSuccess(const QByteArray &response)
{
...
void getTimeInfoWork::handleError(const QString &errorString)
{
    qDebug()<<"send error: "<<errorString;
}

void getTimeInfoWork::handleFail()
{
    getTimeInfoFinished = true;
    qDebug()<<"重传次数过多";
}

void getTimeInfoWork::onCalculateSuccess()
{
    startSending();
}
