...
chooseFaceWidget::chooseFaceWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::chooseFaceWidget)
{
    ui->setupUi(this);
...
chooseFaceWidget::~chooseFaceWidget()
{
    delete ui;
}

void chooseFaceWidget::mousePressEvent(QMouseEvent *event)
{
...
void chooseFaceWidget::mouseMoveEvent(QMouseEvent *event)
{
...
void chooseFaceWidget::screenAdpater()
{
    if(w_fx == 0 || h_fx ==0)
    {
...
void chooseFaceWidget::setupScrollArea()
{
...
void chooseFaceWidget::addPicWidgetToGrid(const QString &uuidStr, const QString &username, QPixmap pixmap)
{
    picWidget* pPicWidget = new picWidget(uuidStr, username, pixmap, containerWidget);
    pPicWidget->curType = chooseFace;
    connect(pPicWidget, &picWidget::sendTmpUserFaceInfo, this, &chooseFaceWidget::sendTmpUserFaceInfo);
...
//void chooseFaceWidget::removePicWidget(picWidget *widget)
//{
//    layout->removeWidget(widget);
//    widget->deleteLater(); // 安全删除
//}

void chooseFaceWidget::clearAllPicWidgets()
{
    if (!layout) return;
...
void chooseFaceWidget::onSendTmpUsrFaceInfo(QList<std::tuple<QString, QString, QPixmap> > list)
{
    clearAllPicWidgets();
    if(list.isEmpty())
    {
...
void chooseFaceWidget::on_btn_return_clicked()
{
    emit returnTmpAuthPage();
}

void chooseFaceWidget::on_btn_min_clicked()
{
    QWidget::showMinimized();
}

void chooseFaceWidget::on_btn_close_clicked()
{
    QWidget::close();
}

void chooseFaceWidget::on_btn_search_clicked()
{
    QString tmpUsername = ui->lineEdit->text();
    if(tmpUsername.isEmpty())
    {
        emit getAllTmpUsrFaceInfo();
        return;
    }
    
    emit getTmpUsrFaceInfo(tmpUsername);
}
